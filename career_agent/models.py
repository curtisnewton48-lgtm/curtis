...
class PortalAnswerPack(BaseModel):
    title: str
    content: str
...
class ModelClient(Protocol):
    ...
    def generate_portal_answers(self, profile: dict[str, str], job: dict[str, str]) -> PortalAnswerPack:
        ...
...
Also include, where profile evidence supports it: leadership, resilience, advocacy/persuasion, ethics, confidentiality, diversity/inclusion, technology/AI, initiative, empathy, and motivation for law.
...
PORTAL_ANSWER_PROMPT = """You are a DeepSeek-powered application portal answer drafting agent for Curtis Newton, a first-class UK law graduate.
Draft concise, truthful, high-quality answers for legal graduate, paralegal, caseworker, and trainee solicitor application portals.
Use only the supplied profile/CV context, job context, Stage 1 fields, and any available Stage 2 research summary/link. Do not invent work experience, grades, dates, awards, languages, or personal circumstances.
Where a portal answer needs a real example that is missing, use a bracketed prompt such as [insert real example of client care].
Produce reusable drafts for common legal application questions, including: why this firm/organisation, why this role, why law, practice-area motivation, commercial/legal awareness, client care, teamwork, resilience, attention to detail, legal research, diversity/inclusion, use of technology/AI, strengths, weaknesses, mitigating circumstances if applicable, and additional information.
For each answer include: question, answer draft, word-count target, evidence used, what Curtis must verify before submitting, and how to tailor it further.
Return only valid JSON with keys: title, content."""
    ...
    def generate_portal_answers(self, profile: dict[str, str], job: dict[str, str]) -> PortalAnswerPack:
        response = httpx.post(
            ...
            json={
                "model": self.model,
                "input": [
                    {"role": "system", "content": PORTAL_ANSWER_PROMPT},
                    {"role": "user", "content": build_user_prompt(profile, job)},
                ],
                "text": {"format": {"type": "json_object"}},
            },
        ...
        payload = response.json()
        output_text = payload.get("output_text") or _extract_openai_output_text(payload)
        return PortalAnswerPack.model_validate_json(output_text)
...
class MistralModelClient:
    ...
    def generate_portal_answers(self, profile: dict[str, str], job: dict[str, str]) -> PortalAnswerPack:
        ...
        if not isinstance(content, str):
            content = json.dumps(content, ensure_ascii=True)
        return PortalAnswerPack.model_validate_json(content)
...
class GeminiModelClient:
    ...
    def generate_portal_answers(self, profile: dict[str, str], job: dict[str, str]) -> PortalAnswerPack:
        content = self._generate_json(PORTAL_ANSWER_PROMPT, build_user_prompt(profile, job))
        return PortalAnswerPack.model_validate_json(_json_object_text(content))
...
def create_model_client(provider: str, model_name: str) -> ModelClient:
    ...
    if provider == "deepseek":
        return DeepSeekModelClient(model_name)
...
class DeepSeekModelClient:
    def __init__(self, model: str) -> None:
        ...
        if not self.api_key:
            raise RuntimeError("DEEPSEEK_API_KEY is required when MODEL_PROVIDER=deepseek.")
        self.model = model

    def _chat_json(self, system_prompt: str, user_prompt: str, timeout: int = 120) -> str:
        response = httpx.post(
    ...
    def score_job(self, profile: dict[str, str], job: dict[str, str]) -> JobFit:
        return JobFit.model_validate_json(self._chat_json(STAGE_ONE_PROMPT, build_user_prompt(profile, job), 60))

    def verify_job(self, profile: dict[str, str], job: dict[str, str]) -> JobVerification:
        return _validate_job_verification(self._chat_json(VERIFICATION_PROMPT, build_user_prompt(profile, job), 60))

    def deep_research(self, profile: dict[str, str], job: dict[str, str]) -> FirmResearch:
        return FirmResearch.model_validate_json(self._chat_json(STAGE_TWO_PROMPT, build_user_prompt(profile, job)))

    def tailor_cv(self, profile: dict[str, str], job: dict[str, str]) -> TailoredCV:
        return TailoredCV.model_validate_json(self._chat_json(CV_TAILORING_PROMPT, build_user_prompt(profile, job)))

    def generate_star_bank(self, profile: dict[str, str], jobs: list[dict[str, str]]) -> STARBank:
        return STARBank.model_validate_json(self._chat_json(STAR_BANK_PROMPT, build_star_bank_prompt(profile, jobs)))

    def generate_portal_answers(self, profile: dict[str, str], job: dict[str, str]) -> PortalAnswerPack:
        return PortalAnswerPack.model_validate_json(self._chat_json(PORTAL_ANSWER_PROMPT, build_user_prompt(profile, job)))
